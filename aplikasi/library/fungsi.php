<?php  
	...
	class oop{
		...
		function login($table, $user, $pass, $type, $alamat){
			...
			if ($cekuser > 0) {
				...
				if ($cekpass > 0) {
					...
					if ($ceklvl > 0) {
						echo "<script>alert('Apakabar $dat[nama_user], sehat selalu dan selamat bekerja.');</script>";
		...
		function logout(){
			session_destroy();
			echo "<script>alert('Terima kasih. Sampai jumpa lagi.');document.location.href='../../';</script>";
		}
	}
